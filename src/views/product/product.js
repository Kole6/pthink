// 产品内页 标题与概要数据
  const titleData = [
    {name:'数据产品',index:0,title:'省市级数据管理与决策分析平台',text:"省市级数据管理与决策分析平台遵循教育部职业教育相关标准规范，通过学校人才培养工作状态数据的自动采集、分析校验，实现关键数据指标的质量监测和数据上报，帮助教育行政管理部门全面掌握区域职业教育的详实情况和运行状态，实现对职业院校的<strong style='font-size:16px'>绩效考核管理</strong>、<strong  style='font-size:16px'>绩效考核指标体系生成</strong>和<strong  style='font-size:16px'>绩效考核数据分析</strong>。"},
    ...
    {name:'质量管理',index:0,title:'智慧魔方质量监测系统',text:'建立学校人才培养质量保证体系，用信息化的工具使质量目标落到实处，质量任务可以拆解，数据过程得以保存，诊改工作有据可依，改进路径有据可循，帮助学校形成常态化自我诊断与改进机制。基于学校现有业务系统（校本数据中心）产生的数据，进行学生、教师、课程、专业、学校五个层面的分析，及时发现、定位学校五个层面的各种问题，提高学校透明化管理。系统支撑全校质量分析体系建设、<strong  style="font-size:16px">绩效考核管理</strong>、<strong  style="font-size:16px">绩效考核指标体系生成</strong>和<strong  style="font-size:16px">绩效考核数据分析</strong>，实时预警，质量报告导出，对异常数据进行针对性处理，抓取学校整体治理数据，辅助校领导进行决策。'},
    {name:'质量管理',index:1,title:'KPI决策分析系统',text:'以学校业务系统（校本数据中心）数据为基础，对学校动态数据进行深度挖掘，将日常管理数据以可视化图形图表的方式动态展示并统计分析，使管理者可以实时动态观测各项状态数据，过程中预警、过程中干预，使所有数据可以追溯源头，查出原因，并可以在一线解决，达成监测质量的目标。系统支撑学校<strong style="font-size:16px">绩效考核管理</strong>、<strong style="font-size:16px">绩效考核指标体系生成</strong>和<strong style="font-size:16px">绩效考核数据分析</strong>。'},
    ...
    {name:'C端应用',index:3,title:'可信教育一证通App',text:''},
  ]
//   产品内页轮播图数据
const picData = [
...
const mainFunctionData = [
    ...
    {name:'质量管理',index:2,num:2,icon:require('@/assets/img/product/detail/icon_p3_3_2.png'),title:'授课资料和题库管理',text:'教师可进行课程资料管理，建立课程文件标签，按课程章节结构分类管理；创建课程试题库，按课程章节结构分类管理。'},
    {name:'质量管理',index:2,num:3,icon:require('@/assets/img/product/detail/icon_p3_3_3.png'),title:'教学质量监控',text:'教学管理人员可按学期查阅课程的授课质量数据、课程的质量情况排行榜，并实现教学质量的全程监控，实时监测、预警提醒。'},
